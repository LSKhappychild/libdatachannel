...
void wsOnMessage(json message, Configuration config, shared_ptr<WebSocket> ws) {
    cout << "(WS)Server: Received message: " << message.dump() << endl;  // Add this debug line
    
    auto it = message.find("id");
    if (it == message.end()) {
        cout << "Server: Missing id field" << endl;
        return;
    }
    ...
    if (it == message.end()) {
        cout << "Server: Missing type field" << endl;
        return;
    }
    string type = it->get<string>();

    cout << "Server: Processing " << type << " message from " << id << endl;

    if (type == "request") {
        cout << "Server: Creating peer connection for " << id << endl;
        clients.emplace(id, createPeerConnection(config, make_weak_ptr(ws), id));
    } else if (type == "answer") {
        cout << "Server: Received answer from " << id << endl;  // Add this debug line
        if (auto jt = clients.find(id); jt != clients.end()) {
            ...
            cout << "Server: Client " << id << " not found" << endl;
        }
    }
}
...
shared_ptr<Client> createPeerConnection(const Configuration &config,
    ...
    pc->onGatheringStateChange(
        ...
        if (state == PeerConnection::GatheringState::Complete) {
            if(auto pc = wpc.lock()) {
                ...
                if (auto ws = wws.lock()) {
                    std::cout << "(WS)Server: Sending answer to client " << id << ": " << message.dump() << std::endl;
    ...
    std::cout << "Server created Datachannel" << std::endl;

    auto dc = pc->createDataChannel("ping-pong");

    dc->onOpen([id, wdc = make_weak_ptr(dc)]() {
        cout << "Server: Data channel opened for client " << id << endl;
    });

    dc->onError([id](string error) {
        cout << "Server: Data channel error for client " << id << ": " << error << endl;
    });

    dc->onClosed([id]() {
        cout << "Server: Data channel closed for client " << id << endl;
    });

    dc->onMessage(nullptr, [id, wdc = make_weak_ptr(dc)](string msg) {
        cout << "(DC)Server: Received message from " << id << ": '" << msg << "'" << endl;
        if (msg != "Ping" && msg != "Pong") {
            cout << "Server: Processing video request: " << msg << endl;
            // TODO: Add video selection logic here
            if (auto dc = wdc.lock()) {
                dc->send("Video request received");
            }
        } else {
            if (auto dc = wdc.lock()) {
                dc->send("Ping");
